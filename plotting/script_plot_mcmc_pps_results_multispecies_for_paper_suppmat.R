...
for (k in 1:730) {
    ...
    if (!is.null(mcmc_samples) && !is.null(mcmc_samples.ss) && !is.null(pps) && !is.null(pps.ss) ) { 
        inds <- c(inds,k)
        T_data <- c()
        ad_data <- c()
        alpha_data <- c()
        gamma_data <- c()
         ...
         ESS.ss <-floor( min(diags.ss[[2]]))
         if (Rhat.ss > 1.1 || ESS.ss < 200){
           ...
           eval(parse(text = paste0(
             'figs[[kk]]  <- ggplot(data=pps_stats_oth) + ggtitle(sprintf("%s site %d %s R-hat %g ESS %d",reef,site,date,Rhat.ss, ESS.ss)) +
           ...
           eval(parse(text = paste0(
             'figsData[[kk]]  <- ggplot(data=pps_stats_oth) + ggtitle(sprintf("%s site %d %s",reef,site,date)) +
                      geom_line(data=filt.rec.traj.proc[[k]],aes(x=T/365,y=HC),linetype = "solid", color="black",size=1) +
                      geom_point(data=filt.rec.traj.proc[[k]],aes(x=T/365,y=HC)) +
                      geom_errorbar(data=filt.rec.traj.proc[[k]],aes(x=T/365,ymin=HC-HC_se,ymax=HC+HC_se)) +
                      geom_line(data=filt.rec.traj.proc[[k]],aes(x=T/365,y=OTH),linetype = "dashed", color="black",size=1) +
           ...
           kk <- kk+1
           figs[[kk]]  <- ggplot(data=alphaD_df_oth) + 
           ...
           kk <- kk+1
           figs[[kk]]  <- ggplot(data=alpha_df_oth) + 
           ...
           kk <- kk+1
           figs[[kk]]  <- ggplot(data=gamma_df_oth) + 
...
for (i in 1:ceil(length(figs)/dims[1]*dims[2])) {
     print(i)
     pdf(file=sprintf('SI_pps_panel%d.pdf',i),width = 33, height = 24)
     grid.arrange(grobs = figs[((i-1)*dims[1]*dims[2]+1):ifelse(i*dims[1]*dims[2] > length(figs),length(figs),i*dims[1]*dims[2])],nrow = dims[1],ncol = dims[2])
     dev.off()
}
