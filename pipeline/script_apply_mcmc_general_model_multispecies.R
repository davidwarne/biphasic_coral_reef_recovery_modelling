...
data <- list(nVisits = length(traj$HC[-1]),         # number of visits excluding initial visit
             c0 = c(A = traj$ACR[1], C = traj$OTH[1]), # cover of initial visit
             ...
             A = traj$ACR[-1],                      # Acroporidea cover value times serie
             C = traj$OTH[-1],        # other hard coral cover value time series
             Serr = cbind(A = traj$ACR_se, C = traj$OTH_se)) # cover standard error time series

low <- c(0.0,0.0,0.0,0,0.01,0,0,0)
up <-  c(0.9,1,2.0,traj$T[length(traj$T)]/365.0,0.9,1,2.0,traj$T[length(traj$T)]/365.0)
...
model <- list(ode_func = general_logistic_twophase,       # RHS for ODE model
                ...
                hyp = c(low[1],up[1],low[2],up[2],low[3],up[3],low[4],up[4],
                        low[5],up[5],low[6],up[6],low[7],up[7],low[8],up[8]),   # prior hyper-parameters
...
#
# # save results
save(filt.rec.traj.proc.mcmc.res,file=paste(PROC_DATA_DIR,"mcmc.sum.pcHC.",
                   sprintf(FILTER_REFMT_OUT_DATA_FMT,max_init,min_final,min_obs)
                                        ,".jobid",num2str(i),".RData",sep=""))

save(filt.rec.traj.proc.post.pred.res,file=paste(PROC_DATA_DIR,"pps.sum.pcHC.",
                   sprintf(FILTER_REFMT_OUT_DATA_FMT,max_init,min_final,min_obs)
                                        ,".jobid",num2str(i),".RData",sep=""))
