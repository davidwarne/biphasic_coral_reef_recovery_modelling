...
loglike <- function(data,model,theta) {
    ...
    if (is.null(model$ode_sol) == FALSE){
        ...
        C_mu <- ode(y = X0, times = c(data$t0,data$ts), 
...
like_sampler <- function(data,model,theta) {
    ...
    ## observation error model
    return(rnorm(length(C_mu[,'C']), mean = C_mu[,'C'], sd = data$Serr))
}
...
model <- list(ode_func = general_logistic_twophase,       # RHS for ODE model
              ...
              lower = c(0.25,0,0,0),                      # lower parameter bounds 
              upper = c(0.75,Inf,Inf,-1),  # upper parameter bounds Note: last entry needs to be updated per trajectory
